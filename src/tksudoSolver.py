...
class sudoParamWindow(tk.Frame):
    ...
    def __init__(self, master=None):
        ...
        menu = tk.Menu(self.newCombo_, tearoff=0)
        self.newCombo_["menu"] = menu

        # Choices in dropdown menu
        menu.add_cascade(label=tkopts.TK_NEW_EMPTY)
        menu.add_cascade(label=tkopts.TK_NEW_EASY)
        menu.add_cascade(label=tkopts.TK_NEW_MEDIUM)
    ...
    def fileName(self, value):
        ...
        if os.path.exists(fullName):
            ...
            if self.backToSingltThreadMode_:
                self.backToSingltThreadMode_ = False
                self.solver_.progressMode = options.options.PROGRESS_SINGLETHREADED
    ...
    def _save(self):
        ...
        if False == done and name is not None:
